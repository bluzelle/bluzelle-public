...
func RegisterDenoms() {
	err := sdk.RegisterDenom(CoinUnit, sdkmath.LegacyOneDec())
	if err != nil {
		panic(err)
	}
	err = sdk.RegisterDenom(MicroCoinUnit, sdkmath.LegacyNewDecWithPrec(1, CoinExponent))
	if err != nil {
		panic(err)
	}
}
...
func SetAddressPrefixes() {
	...
	config.SetAddressVerifier(func(bytes []byte) error {
		if len(bytes) == 0 {
			return sdkerrors.Wrap(errors.ErrUnknownAddress, "addresses cannot be empty")
		}

		if len(bytes) > address.MaxAddrLen {
			return sdkerrors.Wrapf(errors.ErrUnknownAddress, "address max length is %d, got %d", address.MaxAddrLen, len(bytes))
		}

		// TODO: Do we want to allow addresses of lengths other than 20 and 32 bytes?
		if len(bytes) != 20 && len(bytes) != 32 {
			return sdkerrors.Wrapf(errors.ErrUnknownAddress, "address length must be 20 or 32 bytes, got %d", len(bytes))
		}
