...
import { MsgSoftwareUpgrade } from '../../curium/lib/generated/cosmos/upgrade/v1beta1/tx';
...
import { getContainerId, copyToContainer } from '@bluzelle/testing/src/dockerUtils';
...
const getLatestProposalId = (client: any): Promise<string> =>
    getProposals(client)
        .then(res => {
            const ids = res.proposals
                .map(p => Number(p.proposalId))
...
describe('gov module', function () {
    ...
    it('should be able to submit and query a text proposal', () =>
        startSwarmWithClient({
            ...
            .then(proposal => {
                const tempVal = TextProposal.decode(proposal.content.value);
                return expect(tempVal).to.deep.equal(PROPOSAL_VALUE);
            })
    );
...
describe('gov module, local docker', function () {
    ...
    describe('text proposal', () => {
        ...
        it('should be able to get a text proposal to reach deposit period', () =>
            startSwarmWithClient({
                ...
                .then(withCtxAwait("proposalId", client => getLatestProposalId(client.bzSdk)))
                .then(passThroughAwait(client =>
    ...
    describe('software upgrade proposal', () => {
        ...
        it.skip('should apply a software upgrade proposal and confirm that it has been applied', () =>
            startSwarmWithClient({
                config: govTestSwarmConfig,
                isE2E: isE2E()
            })
        ...
        it.skip('should apply a software upgrade proposal to double token supply', () =>
            startSwarmWithClient({
                config: { ...govTestSwarmConfig }
            })
    ...
    describe('parameters change proposal', () => {
        it('should be able to vote on and pass a parameters change proposal', () =>
            startSwarmWithClient({
                ...
                .then(withCtxAwait("proposalId", client =>
                    getProposals(client.bzSdk)
                        .then(res => Math.max(...res.proposals.map(p => Number(p.proposalId))).toString())
                ))
                .then(passThroughAwait(client => vote(client.bzSdk, {
                    proposalId: client.proposalId,
                    voter: client.auth.address,
    ...
    describe('community pool spend proposal', () => {
        ...
        it('should be able to receive funds from a community pool spend proposal', () =>
            startSwarmWithClient({
                ...
                .then(withCtxAwait("proposalId", ctx => getLatestProposalId(ctx.bzSdk)))
                .then(passThroughAwait(client => vote(client.bzSdk, {
                    proposalId: client.proposalId,
                    voter: client.auth.address,
                    option: VoteOption.VOTE_OPTION_YES
...
describe.skip('gov module, local machine', () => {
    ...
    it('should be able to vote on and pass a text proposal', () =>
        newBluzelleClient({
            ...
            .then(withCtxAwait("proposalId", client => getLatestProposalId(client)))
