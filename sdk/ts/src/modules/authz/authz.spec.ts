...
describe('authz module', function () {
    ...
    grantAuthorization(client, testGranter, testGrantee, {
      ...
      .then((res: BluzelleQueryGrantsResponse) => {
        expect(res.grants[0]?.authorization.typeUrl)
          .to
          .equal('/cosmos.authz.v1beta1.GenericAuthorization');
        expect(GenericAuthorization.decode(res.grants[0]?.authorization.value).msg)
          .to
          .equal(msgMapping[MsgType.VERIFY_INVARIANT]);
      })
  );
  ...
  it('delegate msg authorization should be successfully created and executed ', () =>
    grantAuthorization(client, testGranter, testGrantee, {
      ...
      .then((res: BluzelleQueryGrantsResponse) => {
        expect(res.grants[0]?.authorization.typeUrl)
          .to
          .equal('/cosmos.staking.v1beta1.StakeAuthorization');
        expect(StakeAuthorization.decode(res.grants[0]?.authorization.value).authorizationType)
          .to
          .equal(AuthorizationType.AUTHORIZATION_TYPE_DELEGATE);
      })
  ...
  it('undelegate msg authorization should be successfully created and executed', () =>
    grantAuthorization(client, testGranter, testGrantee, {
      ...
      .then((res: BluzelleQueryGrantsResponse) => {
        ...
        expect(StakeAuthorization.decode(res.grants[0]?.authorization.value).authorizationType)
          .to
          .equal(AuthorizationType.AUTHORIZATION_TYPE_UNDELEGATE);
      })
      .then(() => getDelegation(eClient, testGranter, testValAddress)
        .then((delegation: BluzelleDelegationResponse) => createCtx('initialAmount', () => delegation.delegation.shares))
        .catch(() => createCtx('initialAmount', () => 0))
  ...
  it('redelegate msg authorization should be successfully created and executed', () =>
    grantAuthorization(client, testGranter, testGrantee, {
      ...
      .then((res: BluzelleQueryGrantsResponse) => {
        ...
        expect(StakeAuthorization.decode(res.grants[0]?.authorization.value).authorizationType)
          .to
          .equal(AuthorizationType.AUTHORIZATION_TYPE_REDELEGATE);
      })
      ...
      .then((ctx: any) => {
        if (ctx.validators.length >= 2) {
          ...
          return delegate(client, testGranter, testValAddress1, 100, {
            maxGas: 1000000,
            gasPrice: 0.002
          })
  ...
  it(' updateMetadataAuthority authorization should be successfully created and executed.', () => {
    ...
    return createCollection(client, {
      ...
      .then((updatedNftInfo) => {
        expect(updatedNftInfo.metadata?.metadataAuthority)
          .to
          .equal(testGrantee);
      });
  });
  ...
  it(' updateMetadata authorization should be successfully created and executed.', () => {
    ...
    return createCollection(client, {
      ...
      .then((ctx) => getNftInfo(eClient, ctx.nftInfo.id))
      .then((updatedNftInfo) => {
        expect(updatedNftInfo.metadata?.name)
          .to
          .equal('UpdatedTmpMeta');
      });
  });
  ...
  it(' updateMintAuthority authorization should be successfully created and executed.', () => {
    ...
    return createCollection(client, {
      ...
      .then((ctx) => getNftInfo(eClient, ctx.nftInfo.id))
      .then((updatedNftInfo) => {
        expect(updatedNftInfo.metadata?.mintAuthority)
          .to
          .equal(testGrantee);
      });
  });
    ...
    return createCollection(client, {
      ...
      .then((ctx: any) => getNftInfo(eClient, ctx.nftInfo.id)
        .then((updatedNftInfo: any) => {
          expect(ctx.oldInfo.metadata.masterEdition.supply - updatedNftInfo.metadata.masterEdition.supply)
            .to
            .equal(-1);
        })
      );
  ...
  it('grant should be successfully revoked', () =>
    grantAuthorization(client, testGranter, testGrantee, {
      ...
      .then((res: BluzelleQueryGrantsResponse) => {
        ...
        expect(GenericAuthorization.decode(res.grants[0]?.authorization.value).msg)
          .to
          .equal(msgMapping[MsgType.SUBMIT_PROPOSAL]);
      })
      .then(() => revokeAuthorization(client, testGranter, testGrantee, MsgType.SUBMIT_PROPOSAL, {
        maxGas: 100000000,
        gasPrice: 0.002
      }))
      .then(() => queryAuthorizations(client, {
      ...
      .then((res: any) => expect(res.grants.length)
        .to
        .equal(0))
  );
