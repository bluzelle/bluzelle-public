...
export default {
	...
	actions: {
		...
		async QueryHasContent({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryHasContent({...query, 'pagination.key':(<any> value).pagination.next_key})).data
